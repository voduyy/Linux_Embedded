...
static long m_unlockioctl(struct file *filp, unsigned int command, unsigned long arg)
{
    ...
    {
    case LED_ON:
        gpiod_set_value(m_gpio, HIGH);
        pr_info("State of LED %s\n", gpiod_get_value(m_gpio) ? "high" : "low");
        pr_info("Command led on being called \n");
        break;
    case LED_OFF:
        gpiod_set_value(m_gpio, LOW);
        pr_info("State of LED %s\n", gpiod_get_value(m_gpio) ? "high" : "low");
        pr_info("Command led off being called \n");
        break;
    }
    return 0;
}
...
static int gpio_probe(struct platform_device *pdev)
{
    ...
    gpio_dev = &pdev->dev;
    m_gpio = gpiod_get(gpio_dev, "led44", GPIOD_OUT_LOW);
    if (IS_ERR(m_gpio))
    {
